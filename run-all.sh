...
find . -mindepth 1 -maxdepth 1 -type d -print0 | while IFS= read -r -d '' dir; do
  ...
  if [ -f main.py ]; then
    ...
    python3 -m venv env && source env/bin/activate && pip install -r requirements.txt 2>"$LOGS_DIR/$plugin.log" 1>/dev/null
    pip install --no-cache-dir --upgrade --force-reinstall --no-deps ../common/dist/common-1.0-py3-none-any.whl 2>"$LOGS_DIR/$plugin.log" 1>/dev/null
